  ...
  const fetchContacts = async (page: number) => {
    try {
      const response = await fetch(
        `/api/admin/contacts?page=${page}&limit=${pagination.limit}`,
      );
      if (response.ok) {
        const data = await response.json();
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span
                                className={`font-medium text-gray-900 ${contact.status === "new" ? "font-bold" : ""}`}
                              >
                                {contact.name}
                              </span>
                              <span
                                className={`text-xs px-2 py-0.5 rounded-full ${getStatusBadge(contact.status)}`}
                              >
                                {contact.status}
                              </span>
                            </div>
                    ...
                    <div className="flex items-start gap-6">
                      ...
                      <div className="flex-1">
                        <label className="relative cursor-pointer">
                          <div
                            className={`border-2 border-dashed rounded-lg p-4 text-center transition-colors ${uploading ? "border-blue-400 bg-blue-50" : "border-gray-300 hover:border-blue-400"}`}
                          >
                            {uploading ? (
